...
public:
    ...
    X_P_Set get_intersection(int* neighbours, int size){
        ...
        for (int i = 0; i < size; i++){
            ...
            if (in_P(elm)){
                do_swap_as_undoable(get_Pi(new_p_size), elm);
                new_p_size++;
            }
            else if (in_X(elm)){
                do_swap_as_undoable(get_Xi(new_x_size), elm);
                new_x_size++;
            }
        }
    ...
    X_P_Set get_exclusion(int* neighbours, int size){
        ...
        for (int i = 0; i < size; i++){
            ...
            if (in_P(elm)){
                do_swap_as_undoable(get_Pi(new_p_size-1), elm);
                new_p_size--;
            }
            else if (in_X(elm)){
                do_swap_as_undoable(get_Xi(new_x_size-1), elm);
                new_x_size--;
            }
        }
        return X_P_Set(*this, new_x_size, new_p_size, org_undo_size);
    ...
    bool add_exclusion(int elm){
        ...
        do_swap_as_undoable(elm, get_Pi(0));
        P_start++;
        X_size++;
        P_size--;
        return true;
    ...
    void do_swap_as_undoable(int elm_a, int elm_b){
        undo_queue.push_back(elm_b);
        undo_queue.push_back(elm_a);
        do_swap(elm_a, elm_b);
    }

    void undo_changes(){
        while (undo_queue.size() > queue_size){
            int elm_a = undo_queue[undo_queue.size()-1];
            int elm_b = undo_queue[undo_queue.size()-2];
            do_swap(elm_a, elm_b);
            undo_queue.pop_back();
            undo_queue.pop_back();
        }
