...
//  This is the C++ Version of the Document : http://www.kuchaev.com/files/correctImp.py
...
public:
	...
	void loadgraph(Graph& g)
	{
		for (int i = 0; i<g.edges_list.size(); i++){
			mNodes.insert(i);
			mAdjacencyList[i] = set<int>();
			for (int e: g.edges_list[i]){
				mAdjacencyList[i].insert(e);
			}
		}
	}
	...
	std::vector< std::set<int> > find_all_cliques(int min_vertex_in_clique=1)
	{
