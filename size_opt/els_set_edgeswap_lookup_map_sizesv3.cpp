...
public:
    ...
    n(n), X_size(0), P_start(0), P_size(n), lookup(*new vector<int>(n, -1)), vals(*new vector<int>(n, -1)), undo_queue(*new vector<int>()), g(g), elook(*new edge_lookup(g)), edge_sizes(vector<int>(n,-1)){
        for (int i = 0; i<n; i++){
            lookup[i]=i;
            vals[i]=i;
        }
    ...
    X_P_Set(X_P_Set& s, int X_size, int P_size, int org_queue_size) : 
        ...
        g(s.g),
        elook(s.elook),
        edge_sizes(vector<int>(X_size+P_size, -1))
    {};

    //constructor will just use same lookup and val table but change refs to pointers
    X_P_Set(X_P_Set& s, int X_size, int P_size, int org_queue_size, vector<int> new_edge_sizes) : 
        n(s.n), 
        lookup(s.lookup), 
        vals(s.vals), 
        undo_queue(s.undo_queue),
    ...
    X_P_Set get_intersection_iter_list(int* neighbours, int size){
        ...
        vector<int> new_adj_sizes(new_p_size+new_x_size, -1);

        for(int i = 0; i < new_p_size; i++){
            elm = get_Pi(i);
            update_adj_list(elm, new_p_size, new_x_size, new_adj_sizes);
        ...
        return X_P_Set(*this,new_x_size, new_p_size, org_undo_size, new_adj_sizes);
    };
    ...
    void update_adj_list(int elm, int new_p_size, int new_x_size, vector<int>& new_edge_sizes){
        ...
        for (int j = 0; j < edge_list.size(); j++){
            ...
            if (loc >= P_start && loc < P_start+new_p_size){//apply swap for smaller val
                elook.do_edge_swap(elm, edge_list[adj_size], e);
                adj_size++;
            }
            else if (!in_P(e)){
                break;
        ...
        new_edge_sizes[lookup[elm] - P_start + new_x_size] = adj_size;
    ...
    void update_adj_list_single_elm(int source, int elm, int side = 0){ //want to move elm to outer edges of source edgelist
        ...
        if (p_size> edge_list.size()|| p_size> P_size || p_size == 0){
            ...
            printf("source: %d, elm: %d\n", source, elm);
            print_vector(edge_list);
            print_vector(edge_sizes);
            for (int i = 0; i<P_size; i++){
                printf("%d ",get_Pi(i));
            } printf("\n");
            print_everything();
        ...
        edge_list[lookup[source]-P_start+X_size] --;
    }

    int get_intersection_P_size(int elm){
        int setpos = lookup[elm]-P_start+X_size;
        if (edge_sizes[setpos] != -1){
            //printf("using result\n");
            //return edge_sizes[setpos];
        }
        else{
            printf("unable to find shortcut for %d\n", setpos);
            
        }
        int sh = edge_sizes[setpos];
        int real= get_intersection_P_size(g.edges_list[elm].data(), g.edges_list[elm].size());
        printf("using result, gotten: %d, real %d\n",sh,real);
        if (sh != real){
            printf("with %d: in P: %d, in X: %d", elm, in_P(elm), in_X(elm));
        }
        return real;
    }
