...
/*
THIS IS A VERSION OF X_P_edgeswap which reshuffles X,P at the front instead of just P
*/
...
public:
    ...
    X_P_Set get_intersection(int* neighbours, int size){
        ...
        for(int i = 0; i < new_p_size; i++){
            elm = get_Pi(i);
            update_adj_list(elm, new_p_size, new_x_size);
    ...
    void update_adj_list(int elm, int new_p_size, int new_x_size){
        int till = 0;
        auto& edge_list = g.edges_list[elm];
        for (int j = 0; j < edge_list.size(); j++){
            int edge = edge_list[j];
            int loc = lookup[edge];
            if (loc >= P_start-new_x_size && loc < P_start+new_p_size){//apply swap
                swap(edge_list[till], edge_list[j]);
                till++;
            }

            else if (!in_P(edge) && !in_X(edge)){
                break;
            }
        }
    }
    ...
    int get_intersection_P_size(int* neighbours, int size){
        ...
        for (int i = 0; i < size; i++){
            ...
            else if(!in_X(elm)) break;
            #ifdef USE_X_IN_PIVOT_CHOICE
            else new_p_size++;
            #endif
        }
        return new_p_size;
